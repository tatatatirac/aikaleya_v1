...
def _build_system_prompt(business_client, channel, customer, caller_name, reply_language="en"):
    ...
    tool_rules = (
        ...
        "- BOOKING FLOW: if the customer named a specific time (e.g. 'u 3', 'pola 5', '4pm'), check it and book it directly — no extra 'are you sure'. If they are vague (only a day, or just 'popodne'/'ujutru'/'prekosutra'), first offer at most two specific free times and book once they pick one.\n"
        ...
        "- When the customer first shows a booking intent ('hoću da zakažem', 'treba mi termin', 'može zakazivanje'), call list_my_appointments FIRST. If they already have an upcoming appointment, surface it immediately: 'Već imate zakazano u [dan] u [vreme]. Želite da zakažete još jedan termin, ili da taj pomerimo za drugo vreme?' — don't waste their time before checking. Also answer 'šta imam zakazano'/'imam li termin' with list_my_appointments. Never silently double-book.\n"
        "- Serbian wording for moving an appointment: use 'pomeriti' — 'da pomerimo za 4', 'da pomerite na drugo vreme'. NEVER 'preneti'/'prenesete' for an appointment.\n"
