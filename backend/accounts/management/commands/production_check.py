...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if settings.SECRET_KEY in {"dev-only-change-me", "change-this-secret-key", "change-this-to-a-long-random-secret-key"}:
            errors.append("DJANGO_SECRET_KEY mora biti pravi dugačak random secret key.")

        if not settings.ALLOWED_HOSTS or any(host in {"localhost", "127.0.0.1"} for host in settings.ALLOWED_HOSTS):
            warnings.append("DJANGO_ALLOWED_HOSTS treba da sadrži produkcioni domen, bez localhost vrednosti.")
        ...
        if settings.KALEYA_AI_PROVIDER == "anthropic" and not settings.KALEYA_ANTHROPIC_API_KEY:
            errors.append("ANTHROPIC_API_KEY nije podešen.")

        if settings.KALEYA_AI_PROVIDER == "openai" and not settings.KALEYA_OPENAI_API_KEY:
            errors.append("OPENAI_API_KEY nije podešen, a AI_PROVIDER je openai.")

        if not settings.KALEYA_ELEVENLABS_API_KEY:
            warnings.append("ELEVENLABS_API_KEY nije podešen. Glas neće raditi preko ElevenLabs.")

        if not settings.KALEYA_ELEVENLABS_VOICE_ID:
            warnings.append("ELEVENLABS_VOICE_ID nije podešen.")

        demo_admin = User.objects.filter(username="admin@aikaleya.com").first()
        if demo_admin and demo_admin.check_password("admin123"):
            errors.append("Demo admin lozinka admin123 je još aktivna. Na produkciji napravi novog admina ili promeni lozinku.")
        ...
        self.stdout.write(self.style.SUCCESS("Kaleya production check je prošao."))
